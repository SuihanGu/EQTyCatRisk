...
DATA_DIR = ROOT / "data" / "Sample data 1"
MOMENT_CSV = DATA_DIR / "Coupling moment information.csv"
TRACK_CSV = DATA_DIR / "Complete set of typhoon events satisfying coupling conditions.csv"
...
def main() -> None:
    ...
    for idx, row in enumerate(moments):
        ...
        r30_km = to_float(row.get("R30_km"))
        if distance_km is not None and r30_km is not None and distance_km > r30_km:
            continue
        mw, wind_ms = to_float(row.get("Mw")), to_float(row.get("wind_ms"))
        wind_kt = to_float(row.get("wind_kt"))
        ...
        events.append(
            {
                ...
                "windKt": round(wind_kt, 1) if wind_kt is not None else round(wind_ms / 0.514444, 1),
