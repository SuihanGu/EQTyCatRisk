...
export type GridHeatMetric = 'loss' | 'pga' | 'wind'

export const GRID_HEAT_METRICS: Array<{ value: GridHeatMetric; label: string }> = [
  { value: 'pga', label: 'PGA' },
  { value: 'wind', label: 'Wind Speed' },
]

export function gridHeatMetricLabel(metric: GridHeatMetric): string {
  return GRID_HEAT_METRICS.find((item) => item.value === metric)?.label ?? 'Coupled loss'
}

function metricValue(cell: RiskGridCell, metric: GridHeatMetric): number {
  if (metric === 'pga') return cell.pgaGal ?? 0
  if (metric === 'wind') return cell.windMs ?? 0
  return cell.lossJpy
}

function interpolateColor(start: [number, number, number], end: [number, number, number], t: number): string {
  const x = Math.max(0, Math.min(1, t))
  return `rgb(${Math.round(start[0] + (end[0] - start[0]) * x)}, ${Math.round(
    start[1] + (end[1] - start[1]) * x,
  )}, ${Math.round(start[2] + (end[2] - start[2]) * x)})`
}

function heatColor(t: number, metric: GridHeatMetric): string {
  if (metric === 'pga') {
    return interpolateColor([30, 64, 175], [239, 68, 68], t)
  }
  if (metric === 'wind') {
    return interpolateColor([5, 150, 105], [124, 58, 237], t)
  }
  return lossColor(t)
}
...
const GridLossCanvas = L.Layer.extend({
  ...
  initialize(this: any, cells: RiskGridCell[], options?: GridLossOptions) {
    ...
    this._cells = cells
    this._metric = options?.metric ?? 'loss'
    this._maxValue = Math.max(...cells.map((c) => metricValue(c, this._metric)), 1)
  ...
  _cellsInView(this: any): RiskGridCell[] {
    ...
    for (const cell of this._cells as RiskGridCell[]) {
      ...
      if (
        !prev ||
        metricValue(cell, this._metric as GridHeatMetric) >
          metricValue(prev, this._metric as GridHeatMetric)
      ) {
  ...
  _redraw(this: any) {
    ...
    const half = this.options.halfDeg as number
    const maxValue = this._maxValue as number
    const metric = this._metric as GridHeatMetric
    const cells = this._cellsInView()

    for (const cell of cells) {
      const rawValue = metricValue(cell, metric)
      const t = metric === 'loss'
        ? Math.sqrt(Math.max(0, rawValue) / maxValue)
        : Math.max(0, rawValue) / maxValue
      ...
      ctx.fillStyle = heatColor(t, metric)
...
/** 按网格中心着色显示损失、PGA 或风速（单 Canvas） */
export function addGridLossLayer(
  cells: RiskGridCell[],
  layer: L.LayerGroup,
  halfDeg = 0.0041666667,
  metric: GridHeatMetric = 'loss',
): [number, number][] {
  if (!cells.length) return []

  const canvasLayer = new (GridLossCanvas as any)(cells, { halfDeg, minZoomFull: 9, metric })
