...
function epicenterPopupHtml(event: CouplingEvent): string {
  ...
  return `
    <div class="epicenter-popup">
      ...
      <p class="epicenter-popup-mag">Mj ${event.magnitude.toFixed(1)}</p>
...
function createEpicenterIcon(compact = false): L.DivIcon {
  ...
  return L.divIcon({
    ...
    html: `<div class="epicenter-marker${compact ? ' is-compact' : ''}" aria-hidden="true"><span class="epicenter-point"></span></div>`,
    iconSize: [size, size],
