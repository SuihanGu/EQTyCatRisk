...
function buildDescriptions(item: CouplingCatalogItem): string[] {
  const lines: string[] = [
    ...
    `Epicenter ${item.epicenter.lat.toFixed(2)}°N, ${item.epicenter.lng.toFixed(2)}°E, Mj ${item.magnitude.toFixed(2)}`,
  ]
  ...
  if (item.eqTime) lines.push(`Earthquake time (JST) ${item.eqTime}`)
  if (item.tcTime) lines.push(`Typhoon coupling time (JST) ${item.tcTime}`)
  if (item.distanceKm != null) {
    lines.push(`Epicenter–typhoon distance ≈ ${item.distanceKm.toFixed(1)} km`)
  }
  if (item.r30Km != null) {
    lines.push(`R30 ≈ ${item.r30Km.toFixed(1)} km`)
  }
