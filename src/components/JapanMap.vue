...
const props = defineProps<{
  ...
  gridHalfDeg?: number
  heatMetric?: GridHeatMetric
}>()

const emit = defineEmits<{
  select: [id: string]
  'update:heatMetric': [metric: GridHeatMetric]
}>()
...
/** English-label basemap first, with global fallbacks that need no API key. */
const TILE_SOURCES = [
  {
...
function drawOverviewPath(item: CouplingCatalogItem, layer: L.LayerGroup, muted: boolean) {
  ...
  line.bindTooltip(
    `Typhoon ${typhoonCodeFromZid(item.id)} · Coupled Mj ${item.magnitude.toFixed(1)} · ${formatCouplingType(item.couplingType)}`,
  ...
  addPathDirectionArrows(coords, layer, {
    ...
    every: Math.max(1, Math.floor(coords.length / 2)),
    maxArrows: 1,
    size: muted ? 16 : 20,
    opacity: muted ? 0.7 : 1,
  })
...
function drawSelectedDetail(event: CouplingEvent, layer: L.LayerGroup) {
  ...
  if (pathCoords.length > 1) {
    ...
    addPathDirectionArrows(pathCoords, layer, {
      color: '#0f766e',
      every: Math.max(1, Math.floor(pathCoords.length / 2)),
      maxArrows: 1,
      size: 24,
      opacity: 1,
    })
  ...
  if (tc) {
    if (event.r30Km != null && event.r30Km > 0) {
      L.circle([tc.lat, tc.lng], {
        radius: event.r30Km * 1000,
        color: '#0d9488',
        fillColor: '#14b8a6',
        fillOpacity: 0.1,
        weight: 1.5,
        ...
        .bindTooltip(`R30 · ${event.r30Km.toFixed(1)} km`)
        .addTo(layer)
    }
...
function renderLoss() {
  ...
  const bounds = addGridLossLayer(
    grids,
    lossLayer,
    props.gridHalfDeg ?? 0.0041666667,
    props.heatMetric ?? 'loss',
  )
...
<template>
  <section class="map-panel">
    <div class="map-panel-head">
      <h3>{{ lossGrids?.length ? `${gridHeatMetricLabel(heatMetric ?? 'loss')} · Chiba` : 'Japan map' }}</h3>
      <div class="map-panel-head-right">
        <div v-if="lossGrids?.length" class="heat-metric-control" aria-label="Heatmap metric">
          <button
            v-for="metric in GRID_HEAT_METRICS"
            :key="metric.value"
            type="button"
            class="heat-metric-button"
            :class="{ active: (heatMetric ?? 'loss') === metric.value, [`metric-${metric.value}`]: true }"
            @click="emit('update:heatMetric', metric.value)"
          >
            {{ metric.label }}
          </button>
        </div>
        <div v-if="lossGrids?.length" class="heat-legend heat-legend--inline" :class="`heat-legend--${heatMetric ?? 'loss'}`">
          <span class="heat-legend-label">{{ gridHeatMetricLabel(heatMetric ?? 'loss') }}</span>
        ...
        <span class="map-badge">
          {{
            events?.length
              ...
              : lossGrids?.length
                ? `${lossGrids.length.toLocaleString()} grid cells`
                : activeTileCredit
          }}
        </span>
      </div>
    ...
    <p v-else class="map-hint">
      <template v-if="lossGrids?.length">
        Grid cells colored by {{ gridHeatMetricLabel(heatMetric ?? 'loss') }}. Hover for details.
      </template>
